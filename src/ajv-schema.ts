...
import { JsonSchemaType } from '@isdk/match-ex'
 ...
 *
 * This module (and therefore Ajv) is intentionally outside the `@isdk/match-ex`
 * core: consumers which never validate a schema never load it. Importing the
 * `@isdk/match-ex-schema` package registers this class into the engine's
 * registry (see `src/index.ts`).
 */
